...
exec('ps axo pid,ppid,%cpu,pmem,user,group,args', $results);
$processes = [];
...
foreach ($processes as $p) {
    ...
    foreach ($p as $i => $f) {
        if ($i == 0)
            $output .= sprintf('<td>%1$s</td>', $f);
        elseif ($i == 2)
            $output .= sprintf('<td class="cpu">%1$s<ins style="width:%1$s%%"></ins></td>', $f);
        elseif ($i == 3)
            $output .= sprintf('<td class="mem">%1$s<ins style="width="%1$s%%"></ins></td>', $f);
        elseif ($i == 6)
            $output .= sprintf('<td class="command">%1$s</td>', $f);
        else
            $output .= sprintf('<td>%1$s</td>', $f);
    }
    $output .= '</tr>';
...
$(function(){
	...
	$('#ref').on('click', function (){
		...
		$.get('<?php echo $currentpath  ?>?isAjax=1', function(h){
			$('#loading').css('opacity',0);
			$('#process').replaceWith(h);
			$('#cpu').html($('#process').data('cpu'));
			if(!stop) clv= setTimeout(function(){$('#ref').click();}, 2000);
		},'html');
	});
